...
export default function Projects() {
  ...
  const [projects, setProjects] = useState(() => {
    ...
    if (saved) {
      try {
        ...
        const missingDefaults = projectsData.filter(
          def => !parsed.some(p => p.title.toLowerCase() === def.title.toLowerCase())
        );
  ...
  return (
    <div id="projects-page" className="flex flex-col gap-6 sm:gap-8 animate-fadeIn">
      ...
      {/* --- PRACTICAL 4 SHOWCASE BANNER --- */}
      <div className="bg-indigo-900 border border-indigo-800 rounded-3xl p-6 sm:p-8 shadow-sm text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex flex-col gap-2 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-indigo-800 text-indigo-200 border border-indigo-700 w-fit">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            New Lab Added — CO/PO CO2 / PO3, PO5
          </div>
          <h3 className="font-display font-bold text-xl sm:text-2xl text-white">
            Practical 4: Building a RESTful API with Node.js & Express
          </h3>
          <p className="text-xs sm:text-sm text-indigo-200 leading-relaxed">
            Full-stack Express backend with complete CRUD endpoints, global request logging middleware, route-specific ID format validator, and centralized 500/404 error handlers.
          </p>
        </div>

        <Link
          to="/practical-4"
          className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl text-sm font-semibold bg-white hover:bg-neutral-100 text-indigo-950 shadow-md transition-all shrink-0 active:scale-98"
        >
          <span>Open Task API Lab (Practical 4)</span>
          <span>→</span>
        </Link>
      </div>
